...
#+SUBTITLE: for version 2.90.1 (v2.90.1-86-gcc435005b+1)
...
#+TEXINFO: @noindent
This manual is for Magit version 2.90.1 (v2.90.1-86-gcc435005b+1).
...
Magit depends on the ~dash~, ~lv~, ~magit-popup~, ~transient~ and ~with-editor~
libraries which are available from Melpa and Melpa-Stable.  Install
them using ~M-x package-install RET <package> RET~.  Of course you may
also install them manually from their repository.
...
If you haven't installed ~dash~, ~lv~, ~magit-popup~, ~transient~ and
